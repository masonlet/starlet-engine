...
namespace Starlet::Engine {
	...
	bool Window::createWindow(const unsigned int widthIn, const unsigned int heightIn, const char* title) {
		...
		if (!window) return Serializer::error("Window", "createWindow", "Failed to create GLFW window");

		width = widthIn;
		height = heightIn;
		return Serializer::debugLog("Window", "createWindow", "Created window: " + std::string(title) + " - " + std::to_string(width) + " x " + std::to_string(height));
	}
	...
	void Window::setWindowPointer(void* userPointer) const {
		if (userPointer == nullptr) Serializer::error("Window", "setWindowPointer", "User pointer cannot be null.");
	...
	bool Window::switchActiveWindowVisibility() {
		if (!window) return Serializer::error("Window", "switchActiveWindowVisibility", "No active window to switch visibility.");
		...
		return (isVisible == GLFW_TRUE)
			? Serializer::debugLog("Window", "switchWindowVisibility", "Window shown")
			: Serializer::debugLog("Window", "switchWindowVisibility", "Window hidden", false);
	}
	bool Window::switchCursorLock() {
		if (!window) return Serializer::error("Window", "switchCursorLock", "No active window to switch cursor lock.");
		...
		return (cursorMode == GLFW_CURSOR_DISABLED)
			? Serializer::debugLog("Window", "switchCursorLock", "Cursor locked")
			: Serializer::debugLog("Window", "switchCursorLock", "Cursor unlocked", false);
	}
}